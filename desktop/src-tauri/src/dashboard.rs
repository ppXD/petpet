...
use petpet::model::ModelIdent;
use petpet::template::registry::TemplateRegistry;
use petpet::xp::classification::{classify, confidence_as_str};
...
pub struct ModelRow {
    ...
    pub cost_usd: f64,
    /// Tier classification (`frontier` / `mid` / `mini` / `unknown`)
    /// from the shared `xp::classification::classify`. Drives the
    /// tier-coloured badge in the drill-down row.
    pub tier: String,
    /// `exact` / `heuristic` / `unknown` — drives the yellow "GUESSED"
    /// pill when not `exact`. See `xp::classification` for the
    /// decision tree.
    pub confidence: String,
    /// `true` when the bundled / synced registry carried an explicit
    /// entry for this model (vs being classified via the hardcoded
    /// family table or keyword heuristic).
...
async fn build_provider_detail_all_pets(
    ...
    let mut models: Vec<ModelRow> = stats
        .into_iter()
        .map(|r| stats_row_to_model_row(r, provider_id))
        .collect();
    models.sort_by(|a, b| {
        b.cost_usd
...
/// Convert a raw DB stats row into a `ModelRow`, including the shared
/// (tier, confidence) classification.
///
/// Single conversion site for both `build_provider_detail_all_pets`
/// (library-wide aggregate) and `load_provider_models` (per-pet
/// drill-down). The classification mirrors what the XP scorer would
/// apply to the next event for this same model — see
/// `petpet::xp::classification`.
fn stats_row_to_model_row(s: petpet::db::StatsRow, provider_id: ProviderId) -> ModelRow {
    ...
    let total = s.input + s.output + s.cache_read + s.cache_creation + s.reasoning;
    ...
    let in_registry = Registry::bundled().lookup(&s.model).is_some();

    ModelRow {
        model: s.model,
        events: s.events,
        tokens_input: s.input,
        tokens_output: s.output,
        tokens_cache_read: s.cache_read,
        tokens_cache_creation: s.cache_creation,
        tokens_reasoning: s.reasoning,
        tokens_total: total,
        cost_usd,
        tier: tier.as_str().to_string(),
        confidence: confidence_as_str(confidence).to_string(),
        in_registry,
    }
}
